...
app.put('/negocios/:id', authMiddleware, async (req, res) => {
  try {
    const { nombre, direccion, tel, wasa } = req.body;
    if (!nombre) return res.status(400).json({ error: 'nombre requerido' });
    const r = await query('UPDATE negocios SET nombre=COALESCE($1,nombre),direccion=COALESCE($2,direccion),tel=COALESCE($3,tel),wasa=COALESCE($4,wasa) WHERE id=$5 RETURNING id',[nombre,direccion,tel,wasa,req.params.id]);
    if (!r.rows.length) return res.status(404).json({ error: 'No encontrado' });
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/negocios/:id/horario', authMiddleware, async (req, res) => {
  try {
    const { horario } = req.body;
    if (!horario) return res.status(400).json({ error: 'Falta horario' });
    const r = await query('UPDATE negocios SET horario=$1::jsonb WHERE id=$2 RETURNING id',[JSON.stringify(horario),req.params.id]);
    if (!r.rows.length) return res.status(404).json({ error: 'No encontrado' });
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
